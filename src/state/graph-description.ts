...
export function describeWorkflow(
  ...
  for (let stepIndex = 0; stepIndex < steps.length; stepIndex++) {
    ...
    if (step.kind === "check") {
      ...
      // Пауза перед вторым касанием живёт не отдельным этапом, а прямо в этой
      // проверке (спека §5): длительность берётся у БЛИЖАЙШЕЙ последующей
      // повторной волны — той самой, ради которой пауза и выдерживается. Тег
      // строится ровно так же, как `waitTag` волны ниже (`waitPhrase` +
      // node-fields/none по `graphEditable`), чтобы пилюля паузы осталась той
      // же самой нодой ожидания с тем же адресом правки (§2.12).
      let pauseTag: DescriptionTag | undefined;
      for (let j = stepIndex + 1; j < steps.length; j++) {
        const ahead = steps[j];
        if (ahead.kind !== "wave" || !ahead.wave.repeatsPrevious) continue;
        const waitNode = ahead.wave.waitBefore;
      ...
      stages.push({
        ...
        body: pauseTag
          ...
          : [t(`${reacted} Остальным делаем второе касание.`)],
      });
      continue;
    }
    ...
    if (isFork) {
      ...
      // Повтор с разделяющей паузой: паузу и уход отреагировавших уже описала
      // слитая «Проверка реакции и пауза» выше, поэтому здесь — короткая
      // подводка к той же таблице, БЕЗ повторного упоминания паузы.
