...
export function channelTemplateParams(channel: Channel): NodeParams {
  switch (channel) {
    ...
    case "email":
      // Fix: раньше это тело («Мы подготовили для вас персональное
      // предложение.») не совпадало ни с одним письмом `email-directory.ts` —
      // шаблон никогда не резолвился, и пилюля названия шаблона на карточке
      // кампании не рисовалась (пока пилюля рендерилась только при резолве).
      // #2 — совпадает по телу с пресетом `tpl_eml_offer` («Персональный
      // оффер»), поэтому селект «Шаблон» сразу показывает имя привязанного
      // шаблона — так же, как у sms/push.
      return {
        kind: "email",
    ...
    case "ivr":
      // Fix: раньше IVR не имело НИ ОДНОГО библиотечного шаблона — «Персональное
      // предложение» не могло совпасть ни с чем в принципе. #2 — совпадает по
      // сценарию с пресетом `tpl_ivr_greeting` («Звонок — приветствие»), так же,
      // как у sms/push/email.
      return {
        kind: "ivr",
        scenario: "Приветствие → проверка интереса → перевод на оператора",
        voiceType: "neutral",
      };
  }
}
