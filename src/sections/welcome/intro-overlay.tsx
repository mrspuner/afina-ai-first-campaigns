 ...
 * жёлтым контуром впритык (1px, без отступа). На шаге подсказок каждый чип
...
export function IntroOverlay({ onDismiss }: { onDismiss: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Тонкие подсветки обсуждаемых элементов — контур впритык (1px). */}
      <AnimatePresence initial={false}>
        {geom.spots.map((s, i) => (
          <motion.div
            key={`spot-${step}-${i}`}
...
/** Тонкий жёлтый контур (1px, впритык) + мягкое свечение (как было). */
const SPOT_RING =
  "0 0 0 1px var(--brand), 0 0 24px 6px color-mix(in srgb, var(--brand) 38%, transparent)";
