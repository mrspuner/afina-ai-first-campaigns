...
describe("вид пилюли", () => {
  ...
  // Вертикальный паддинг пилюли остаётся нулевым — и это НЕ недосмотр.
  // Правку «минимум 4px» пробовали (`py-1`) и откатили по решению владельца:
  // пилюля вырастала в высоту и читалась раздутой, а в абзаце с несколькими
  // тегами (перечисление триггеров) обёрнутые строки слипались вплотную —
  // зазор, ради которого контейнер описания несёт `leading-[1.75]`, съедался
  // целиком. Высоту пилюле задаёт line-height строки, а не собственный паддинг.
  it("не раздувает строку собственным вертикальным паддингом", () => {
    render(<DescriptionTagPill tag={{ id: "t", label: "186 255 строк", target: { kind: "none" } }} />);
    const box = screen.getByText("186 255 строк").parentElement as HTMLElement;
    expect(box.className).toContain("py-0");
    expect(box.className).not.toContain("py-1");
  });
});
