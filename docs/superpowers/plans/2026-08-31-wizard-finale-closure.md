...
- Modify: `src/state/graph-waves.test.ts` — `:72, :85, :101` и соседние тесты строят сегментные графы через `createTemplate(<тип>, "new", [каналы])`
- Modify: `src/state/graph-description.test.ts` — `:291, :603` то же через `describeWorkflow`
- Test: `src/state/workflow-templates.test.ts` (новый it)

**НЕ трогать (решение контроллера, преflight-скан):** продакшн-логику по `by === "segment"` — ни `graph-waves.ts:240` (`segmentWaves`), ни ветку защищённого сегментного сплита в `mergeChannelNodes`. Сегментные сплиты остаются легальными нодами: их ставит пользователь и ИИ. Исчезает только их генерация.
...
- [ ] **Step 5a: Мигрировать `graph-waves.test.ts` и `graph-description.test.ts`**

Эти файлы строят сегментные графы напрямую и после правки получат линейные. Приём миграции тот же, что выше: фикстуру брать с legacy-пути `createTemplate("Апсейл", "new")` / `createTemplate("Удержание", "new")` **без аргумента каналов** — он по-прежнему несёт сплит `by:"segment"` (см. `upsellTemplate:126`, `retentionTemplate:213`).

Тест, чья предпосылка держится именно на трёх параллельных комм-юнитах из генерации (например «из нескольких параллельных пауз waitBefore берёт ПЕРВУЮ по BFS», `graph-waves.test.ts:101`) и на legacy-шаблоне не воспроизводится, — удалить. **Каждое удаление обосновать в отчёте отдельной строкой:** какой тест, какая у него была предпосылка, почему она больше не существует нигде в продукте.

Логику `segmentWaves` (`graph-waves.ts:240`) не трогать — она обслуживает сплиты, поставленные вручную и ИИ.
...
> **Отложено.** Эта задача НЕ выполнена в ветке `feature/wizard-finale`: Step 1
> (воспроизведение) не дал результата несмотря на существенные попытки — баг
> не воспроизведён, и по брифу дальше по шагам (Step 2/3, фикс) идти было
> нельзя. Рабочая гипотеза про `backdrop-filter` из Step 2 при этом
> опровергнута побочными замерами: без блюра остаточная прозрачность панели
> читается ЛУЧШЕ, а не хуже — снимать `backdrop-blur` не будет фиксом. Задача
> припаркована, кода по ней в ветке нет.
